...
def selBest(individuals, k, n, fit_attr="fitness"):
    """Select the *k* best individuals among the input *individuals*. The
    list returned contains references to the input *individuals*.
    ...
    :param k: The number of individuals to select.
    :param fit_attr: The attribute of individuals to use as selection criterion.
    :returns: A tuple containing the list of the k best individuals and the number of selected individuals with an original index over 350.
    """
    # Sort the individuals based on the fitness attribute
    best_individuals = sorted(individuals, key=attrgetter(fit_attr), reverse=True)[:k]

    # Function to find the index from the end to the beginning
    def find_last_index(ind, lst):
        for idx in range(len(lst) - 1, -1, -1):
            if lst[idx] == ind:
                return idx
        return -1

    # Count how many of the selected individuals have an index over 350 in the original list
    count_over_n = sum(1 for ind in best_individuals if find_last_index(ind, individuals) > n)

    # Return the best individuals and the count
    return best_individuals, count_over_n
